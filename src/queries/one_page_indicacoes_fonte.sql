...
-- Renovações ficam só no card Renov. — mesmo com fonte Indicação, não entram
-- aqui. Pode sobrepor Novos (`tipo_venda = 'Novo cliente'` + fonte Indicação).
--
-- Regra:
...
--     AND tipo_venda IS DISTINCT FROM 'Renovação'
--
...
WHERE d.stage IN ('Ganho', 'Fechado Ganho')
  ...
  AND d.tipo_venda IS DISTINCT FROM 'Renovação'
