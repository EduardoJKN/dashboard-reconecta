#!/usr/bin/env python
"""Benchmark e equivalência — prevendas_por_sdr v1 vs v2 (somente leitura).

Uso:
  python scripts/benchmark_prevendas_por_sdr.py
  python scripts/benchmark_prevendas_por_sdr.py --scenario mes_atual --runs 3
"""
...
HOJE = date(2026, 6, 22)
SQL_V1 = "prevendas_por_sdr.sql"
SQL_V2 = "prevendas_por_sdr_v2.sql"
...
SCENARIOS: dict[str, tuple[date, date]] = {
    ...
    "periodo_antigo_sem_dados": (date(2018, 1, 1), date(2018, 1, 31)),
    "futuro_sem_vendas": (date(2030, 1, 1), date(2030, 1, 31)),
}

SCENARIO_ALIASES = {"7_dias": "ultimos_7_dias", "60_dias": "ultimos_60_dias"}

KEY_COLS = ["sdr", "fonte_sdr"]
INT_COLS = [
    "agendamentos_criados", "agendamentos", "agendamentos_mais_12",
    "agendamentos_menos_12", "comparecimentos", "cancelamentos",
    "cancelados", "vencidos", "vendas", "vendas_novas",
]
MONEY_COLS = ["montante", "receita"]
METRIC_COLS = INT_COLS + MONEY_COLS
MONEY_TOL = 0.01
...
def _compare_equiv(df1: pd.DataFrame, df2: pd.DataFrame) -> dict:
    ...
    keys1 = set(zip(n1["sdr"], n1["fonte_sdr"])) if len(n1) else set()
    keys2 = set(zip(n2["sdr"], n2["fonte_sdr"])) if len(n2) else set()
    only1 = keys1 - keys2
    only2 = keys2 - keys1
    if only1 or only2:
        result["pass"] = False
    ...
    for col in METRIC_COLS:
        ...
        result["totals_v1"][col] = float(v1s.sum())
        result["totals_v2"][col] = float(v2s.sum())
...
def main() -> None:
    ...
    print("Benchmark + equivalência — prevendas_por_sdr v1 vs v2")
    ...
    print()
    if all_pass:
        print("APROVAÇÃO: v2 pode substituir v1 no CP3-B (equivalência OK em todos os cenários).")
    else:
