#!/usr/bin/env python
"""EXPLAIN (ANALYZE) — prevendas_por_sdr.sql (somente leitura).

Uso:
  python scripts/explain_prevendas_por_sdr.py
  python scripts/explain_prevendas_por_sdr.py --scenario mes_atual
  python scripts/explain_prevendas_por_sdr.py --sql v2
"""
...
SQL_FILES = {
    "v1": "prevendas_por_sdr.sql",
    "v2": "prevendas_por_sdr_v2.sql",
}
...
def _suggest_rewrite(plan: str, sql_label: str) -> list[str]:
    tips: list[str] = []
    if "zoho_activities" in plan and plan.count("zoho_activities") >= 2:
        tips.append(
            "Dois ou mais acessos a `zoho_activities` — unificar em scan único "
            "com filtro (start_datetime OR created_time) no período."
        )
    if "ext_reconecta.leads" in plan or "leads l" in plan.lower():
        if "Seq Scan" in plan and "leads" in plan:
            tips.append(
                "`ext_reconecta.leads` com Seq Scan amplo — restringir a "
                "deal_ids do período antes do DISTINCT ON."
            )
    if "zoho_deals" in plan and plan.count("Seq Scan on zoho_deals") >= 1:
        tips.append(
            "`zoho_deals` sem filtro de período na classificação — limitar "
            "a deals referenciados por activities/vendas do recorte."
        )
    if not tips:
        tips.append(
            "Revisar joins repetidos e CTEs que materializam bases maiores "
            "que o necessário."
        )
    if sql_label == "v2":
        tips.insert(0, "v2 já aplica scan único de activities e escopo de deals.")
    return tips
...
def main() -> None:
    ...
    print("EXPLAIN ANALYZE — prevendas_por_sdr")
    ...
    for sk in sql_keys:
        ...
        for label in labels:
            ...
            if args.scenario == "all" and label != labels[-1]:
                print()

            if label == labels[-1] and sk != sql_keys[-1]:
                pass
